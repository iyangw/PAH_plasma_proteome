...
QDA = QuadraticDiscriminantAnalysis(reg_param=best_params['reg_param'])

QDA.fit(x_discovery, y_discovery)

print(f"Accuracy Discovery: {QDA.score(x_discovery, y_discovery)}")
print(f"Accuracy Test: {QDA.score(x_test, y_test)}")

y_pred_discovery = QDA.predict(x_discovery)
