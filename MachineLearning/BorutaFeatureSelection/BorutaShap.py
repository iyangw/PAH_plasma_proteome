...
# binom_test was replaced by binomtest
...
class BorutaShap:
    ...
    def explain(self):
        ...
        """
        # TreeExplainer can be applied to tree or boosting algorithms
        ...
        # Added by iyangw
        # LinearExplainer can be applied to linear algorithms
        # XGBoost gblinear need LinearExplainer
       # explainer = shap.LinearExplainer(self.model, self.X_boruta)

        # Added by iyangw
        # KernelExplainer can be applied to other algorithms that are not suitable to TreeExplainer or LinearExplainer
        explainer = shap.KernelExplainer(self.model.predict_proba, self.X_boruta)
        ...
        if self.sample:
            ...
            if self.classification:
                ...
                # Revised by iyangw
                # The results of function shap_values contain different format.
                # For TreeExplainer of tree classifier and KernalExplainer, [:, :, 1] should be added.
                self.shap_values = np.array(explainer.shap_values(self.find_sample())[:, :, 1])
                # For TreeExplainer of boosting classifier and LinearExplainer, [:, :, 1] should be deleted.
                ...
                # The results of function shap_values contain different format.
                # For TreeExplainer of tree classifier and KernalExplainer, [:, :, 1] should be added.
                self.shap_values = explainer.shap_values(self.find_sample())[:, :, 1]
                # For TreeExplainer of boosting classifier and LinearExplainer, [:, :, 1] should be deleted.
            ...
            if self.classification:
                ...
                # The results of function shap_values contain different format.
                # For TreeExplainer of tree classifier and KernalExplainer, [:, :, 1] should be added.
                self.shap_values = np.array(explainer.shap_values(self.X_boruta)[:, :, 1])
                # For TreeExplainer of boosting classifier and LinearExplainer, [:, :, 1] should be deleted.
                ...
                # The results of function shap_values contain different format.
                # For TreeExplainer of tree classifier and KernalExplainer, [:, :, 1] should be added.
                self.shap_values = explainer.shap_values(self.X_boruta)[:, :, 1]
                # For TreeExplainer of boosting classifier and LinearExplainer, [:, :, 1] should be deleted.
    ...
    def binomial_H0_test(array, n, p, alternative):
        ...
        """
        # binom_test was replaced by binomtest
        # Revised by iyangw to extract pvalues in binomtest
        return [result.pvalue for result in [binomtest(x, n=n, p=p, alternative=alternative) for x in array]]
    ...
    def test_features(self, iteration):
        ...
        # Added by iyangw to make numbers in hit integers instead of floats
        self.hits = [int(hit) for hit in self.hits if isinstance(hit, (int, float)) and hit >= 0]
