...
def objective(trial):
    ...
    ETsHyper = ExtraTreesClassifier(n_estimators=n_estimators,
                                    max_depth=max_depth,
    ...
    score = cross_val_score(ETsHyper, x_discovery, y_discovery, cv=kf, scoring='accuracy')
    return score.mean()
...
ETs.fit(x_discovery, y_discovery)

print(f"Accuracy Discovery: {ETs.score(x_discovery, y_discovery)}")
print(f"Accuracy Test: {ETs.score(x_test, y_test)}")

y_pred_discovery = ETs.predict(x_discovery)
y_pred_test = ETs.predict(x_test)
...
# 预测概率
y_prob_discovery = ETs.predict_proba(x_discovery)[:, 1]
y_prob_test = ETs.predict_proba(x_test)[:, 1]
