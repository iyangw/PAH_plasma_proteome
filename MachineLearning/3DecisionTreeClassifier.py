...
def objective(trial):
    ...
    DTHyper = DecisionTreeClassifier(
        criterion=criterion,
        max_depth=max_depth,
    ...
    score = cross_val_score(DTHyper, x_discovery, y_discovery, cv=kf, scoring='accuracy')
    return score.mean()
...
DT.fit(x_discovery, y_discovery)

print(f"Accuracy Discovery: {DT.score(x_discovery, y_discovery)}")
print(f"Accuracy Test: {DT.score(x_test, y_test)}")

y_pred_discovery = DT.predict(x_discovery)
...
# 预测概率
y_prob_discovery = DT.predict_proba(x_discovery)[:, 1]
